...
// import BodyScanner from "../src/BodyScanner,jsx";
// import {
...
// const MEASURE_FIELDS = ["Chest / Bust", "Waist", "Hips", "Shoulder", "Sleeve length", "Garment length", "Neck", "Forearm length"];const PICKUP_SLOTS = ["Today, 4–6 PM", "Today, 6–8 PM", "Tomorrow, 10 AM–12 PM", "Tomorrow, 4–6 PM"];
...
import React, { useState, useEffect, useRef } from "react";
import BodyScanner from "./BodyScanner.jsx";
import {
  Search, MapPin, Star, ChevronLeft, ChevronRight, ChevronDown, Plus, Minus,
  Check, Clock, Phone, ShoppingBag, Package, User, X, Zap, Scissors, Ruler,
  Shirt, Crown, Sparkles, Sofa, Home, Truck, CheckCircle2, Circle, Pencil,
...
const MEASURE_FIELDS = ["Chest / Bust", "Waist", "Hips", "Shoulder", "Sleeve length", "Garment length", "Neck", "Forearm length"];
  ...
  return (
    <SheetShell title={editing ? "Edit measurements" : "New measurement profile"} onClose={onClose}>
      ...
      <button type="button" onClick={() => setScan(true)}
        style={{ width: "100%", border: `1.5px solid ${C.magenta}`, background: C.tint, color: C.magenta,
          borderRadius: 13, padding: "12px", fontWeight: 800, fontSize: 13.5, cursor: "pointer", fontFamily: "inherit",
          display: "flex", alignItems: "center", justifyContent: "center", gap: 7, marginBottom: 16 }}>
        <Ruler size={16} /> Scan with camera
      </button>
      ...
      {scan && (
        <BodyScanner
          ...
          onApply={(vals) => { setValues((v) => ({ ...v, ...vals })); setScan(false); }}
        />
      )}
    </SheetShell>
  );
